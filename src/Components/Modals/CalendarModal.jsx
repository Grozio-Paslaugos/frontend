...
const CalendarModal = ({ opened, onClose, procedureId, bookingId, isEditMode }) => {
    ...
    if (isEditMode && bookingId) {
      ...
      const fetchBookingDetails = async () => {
        try {
          ...
          setSelectedDate(new Date(data.bookingDatetime));
          const time = new Date(data.bookingDatetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log(isEditMode ? 'Booking updated successfully:' : 'Booking created successfully:', response.data);
      onClose();
    } catch (error) {
      console.error(isEditMode ? 'Error updating booking:' : 'Error booking procedure:', error.response?.data || error.message);
    }
  };
